...
if __name__=="__main__":                        #Test Functions
    ...
    tfile = r.TFile.Open("output/CorrelationFunctionHistograms.root","NEW")
    histDD.Write()
    histRR.Write()
    histDpR.Write()
    histEp.Write()
    ...
    histRR.Draw("same")
    histDpR.Draw("same")
    c.Print("output/CorrelationHistogramDatRan.pdf")
    histEp.Draw()
    c.Print("output/CorrelationHistogramTotal.pdf")
    histDD.Divide(histRR)
    histDD.Draw()
    c.Print("output/CorrelationDDRR.pdf")
